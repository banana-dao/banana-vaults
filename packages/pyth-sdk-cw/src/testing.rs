use {
    ...
    cosmwasm_std::{
        from_json, to_json_binary, Binary, Coin, ContractResult, QuerierResult, SystemError,
        SystemResult,
    },
        ...
        match query_msg {
            Ok(QueryMsg::PriceFeed { id }) => match self.feeds.get(&id) {
                Some(feed) => SystemResult::Ok(
                    to_json_binary(&PriceFeedResponse { price_feed: *feed }).into(),
                ),
            ...
            Ok(QueryMsg::GetUpdateFee { vaas }) => {
                let new_amount = self
                    ...
                    .unwrap();
                SystemResult::Ok(
                    to_json_binary(&Coin::new(new_amount, &self.fee_per_vaa.denom)).into(),
                )
            }
            #[cfg(feature = "osmosis")]
